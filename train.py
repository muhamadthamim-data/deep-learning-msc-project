...
save_dir = "/content/drive/MyDrive/SRGAN/samples"
tlx.files.exists_or_mkdir(save_dir)
checkpoint_dir = "/content/drive/MyDrive/SRGAN/models"
...
def calculate_psnr(hr, sr):
    # Ensure that hr and sr are in the range [0, 255]
    hr = np.clip(hr, 0, 255)
    sr = np.clip(sr, 0, 255)

    # Calculate PSNR
    psnr_value = peak_signal_noise_ratio(hr, sr)
    return psnr_value
...
def train():
    ...
    for epoch in range(n_epoch):
        for step, (lr_patch, hr_patch) in enumerate(train_ds):
            ...
            if (step + 1) % 52 == 0:  # Calculate PSNR every 100 steps (adjust as needed)
                sample_hr_patch = hr_patch[:4]  # Select a sample of images for PSNR calculation
                generated_hr_patch = G(lr_patch[:4]).numpy()
                psnr_value = calculate_psnr(sample_hr_patch, generated_hr_patch)
                print("Step: [{}/{}] PSNR: {:.2f} dB".format(step + 1, n_step_epoch, psnr_value))
        ...
        if (epoch != 0) and (epoch % 1 == 0):
            G.save_weights(os.path.join(checkpoint_dir, 'srgan-g-tensorflow.npz'), format='npz_dict')
            D.save_weights(os.path.join(checkpoint_dir, 'srgan-d-tensorflow.npz'), format='npz_dict')
...
def evaluate():
    ...
    G.load_weights(os.path.join(checkpoint_dir, 'srgan-g-tensorflow.npz'), format='npz_dict')
    G.set_eval()
    ...
    # Compute PSNR
    psnr_value = peak_signal_noise_ratio(valid_hr_img, out)
    ...
    lr_img = Image.open(os.path.join(save_dir, 'valid_lr.png'))
    hr_cubic_img = Image.open(os.path.join(save_dir, 'valid_hr_cubic.png'))
    gen_hr_img = Image.open(os.path.join(save_dir, 'valid_gen.png'))
    hr_img = Image.open(os.path.join(save_dir, 'valid_hr.png'))
